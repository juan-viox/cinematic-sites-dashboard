    ...
    <div className="max-w-5xl mx-auto px-6 py-10">
      <div className="mb-8">
        ...
        <p className="text-muted text-sm mt-1">
          Social media management and content marketing via Blotato
        </p>
      </div>
        ...
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-sm text-muted mt-1">
              Manage all social media platforms from one dashboard via blotato.com
            </p>
          </div>
            ...
            return (
              <div
                ...
                className={`p-4 rounded-lg border ${
                  handle ? 'border-success/30 bg-success/5' : 'border-border bg-surface-2'
                }`}
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold ${
                      handle ? 'bg-success/20 text-success' : 'bg-surface text-muted'
                    }`}>
                  ...
                  <span className={`text-xs ${handle ? 'text-success' : 'text-muted'}`}>
                    {handle ? 'Connected' : 'Not linked'}
                  </span>
                </div>
              </div>
            );
      ...
      {connectedPlatforms.length > 0 && (
        <div className="mt-8 p-4 rounded-xl bg-surface-2 border border-border">
          <div className="text-xs text-muted">
            {connectedPlatforms.length} platform{connectedPlatforms.length > 1 ? 's' : ''} linked:
